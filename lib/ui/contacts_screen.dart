...
class _ContactsScreenState extends State<ContactsScreen> {
  ...
  Future<void> _showContactDialog({Contact? contact}) async {
    ...
    final tagsController = TextEditingController(text: contact?.tags.join(', ') ?? '');
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Telefone (com código do país)'),
    ...
    if (await Permission.contacts.request().isGranted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('$addedCount contatos importados do dispositivo!')),
        );
        _loadContacts();
      }
    } else {
  ...
  Future<void> _deleteSelected() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Deseja realmente excluir ${_selectedIds.length} contato(s)?"),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isSelectionMode ? "${_selectedIds.length} selecionados" : 'Meus Contatos'),
               ...
               ? const Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text('Nenhum contato ainda.', style: TextStyle(fontSize: 18, color: Colors.grey)),
                    ],
                  ),
                )
              : ListView.builder(
                            ...
                            Padding(
                              ...
                              child: Wrap(
                                ...
                                children: contact.tags.map((tag) => Container(
                                  ...
                                  child: Text(tag, style: TextStyle(fontSize: 10, color: Theme.of(context).colorScheme.onSecondaryContainer)),
                                )).toList(),
                              ),
                            ),
                        ],
         ...
         content: SizedBox(
           ...
           child: SingleChildScrollView(
             child: Column(
               ...
               children: [
                 const Text("Cole os contatos no formato: Nome, Telefone (um por linha)"),
                ...
                if (text.isNotEmpty) {
                   ...
                   for (var line in lines) {
                     ...
                     if (parts.length >= 2) {
                       ...
                       if (phone.isNotEmpty) {
                         final newContact = Contact(
                            id: DateTime.now().microsecondsSinceEpoch.toString(), // micro to avoid collision in loop
                            name: name,
                            phone: phone,
                         );
                         await _repository.saveContact(newContact);
