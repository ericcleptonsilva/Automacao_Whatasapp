...
class _AutoReplyScreenState extends State<AutoReplyScreen> {
  ...
  final TextEditingController _absenceMessageController =
  ...
  // Showcase Keys
  final GlobalKey _keyAbsence = GlobalKey();
  final GlobalKey _keyFloating = GlobalKey();
  final GlobalKey _keyAi = GlobalKey();
  final GlobalKey _keyAddRule = GlobalKey();
  ...
  Future<void> _testAIConnection() async {
    ...
    try {
      ...
      final response = await aiService.generateReply(
        "Olá, isto é um teste de conexão.",
      );
      ...
      if (response != null && response.isNotEmpty) {
        ...
        messenger.showSnackBar(
          const SnackBar(
            content: Text(
              "Falha: A I.A. não retornou resposta. Verifique a chave e o provedor.",
            ),
          ),
        );
      }
  ...
  Future<void> _exportLogs() async {
    ...
    if (path != null) {
      await Share.shareXFiles([
        XFile(path),
      ], subject: 'Logs de Diagnóstico de Erros - Bot WhatsApp');
    } else {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _startTourIfNeeded() async {
    ...
    final isFirstLaunch = prefs.getBool('first_launch_autoreply_tour') ?? true;
    if (isFirstLaunch && mounted) {
      ...
      await prefs.setBool('first_launch_autoreply_tour', false);
    }
  }
  ...
  Future<void> _loadSettings() async {
    ...
    if (aiModel == 'google/gemini-2.0-flash-lite-preview-02-05:free' ||
        aiModel == 'google/gemini-2.0-flash-lite-preview') {
  ...
  Future<void> _saveAbsenceSettings() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Configurações de Ausência salvas!")),
    );
  }
        ...
        if (mounted) {
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: const Text(
                "O botão flutuante requer a permissão 'Sobrepor a outros apps' para aparecer em cima do WhatsApp. Você será redirecionado para a tela de configurações para habilitá-la.",
              ),
              actions: [
                TextButton(
                  onPressed: () => Navigator.pop(context),
  ...
  Future<void> _addOrEditRule({AutoReplyRule? existingRule, int? index}) async {
    final keywordController = TextEditingController(
      text: existingRule?.keyword ?? '',
    );
    final replyController = TextEditingController(
      text: existingRule?.replyMessage ?? '',
    );
    ...
    return ShowCaseWidget(
      builder: (context) => Scaffold(
            ...
            : ListView(
                ...
                children: [
                  ...
                  Showcase(
                    ...
                    title: "Estou Ocupado",
                    description:
                        "O Modo Ausência responde automaticamente a QUALQUER mensagem. Ideal para quando você não pode atender ninguém.",
                  ...
                  if (_isAbsenceEnabled) ...[
                    TextField(
                      ...
                      maxLines: 3,
                      onChanged: (_) => _repository.setAutoReplyMessage(
                        _absenceMessageController.text,
                      ), // Auto save on type might be too much, but OK for now or add explicit save
                    ),
                    const SizedBox(height: 10),
                    ElevatedButton(
                      onPressed: _saveAbsenceSettings,
                  ...
                  Showcase(
                    ...
                    description:
                        "Ative um botão que aparece por cima do WhatsApp. Ele permite ligar/desligar o bot sem sair da conversa.",
                    child: SwitchListTile(
                      title: const Text("Botão de Controle Flutuante"),
                      subtitle: const Text(
                        "Mostra um botão sobre o WhatsApp para ativar/desativar o bot",
                      ),
                  ...
                  Showcase(
                    ...
                    title: "Poder da I.A.",
                    description:
                        "Se o bot não encontrar uma regra exata, a Inteligência Artificial pode gerar uma resposta humana e inteligente para você.",
                    child: SwitchListTile(
                      title: const Text("Ativar I.A. (Resposta Inteligente)"),
                      subtitle: const Text(
                        "Responde quando nenhuma regra for encontrada",
                      ),
                      value: _isAiEnabled,
                      onChanged: (val) {
                  ...
                  if (_isAiEnabled) ...[
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        ...
                        DropdownMenuItem(
                          value: "OpenRouter",
                          child: Text("OpenRouter (Múltiplas IAs)"),
                        ),
                      ],
                      onChanged: (val) {
                        setState(() {
                          ...
                          final validModels = _getModelsForProvider(
                            _aiProvider,
                          ).map((m) => m.value).toList();
                          if (!validModels.contains(_aiModel)) {
                            if (_aiProvider == 'Gemini') {
                              _aiModel = 'gemini-2.0-flash';
                            }
                            if (_aiProvider == 'Claude') {
                              _aiModel = 'claude-3-haiku-20240307';
                            }
                            if (_aiProvider == 'DeepSeek') {
                              _aiModel = 'deepseek-chat';
                            }
                            if (_aiProvider == 'Groq') {
                              _aiModel = 'llama-3.1-8b-instant';
                            }
                            if (_aiProvider == 'Mistral') {
                              _aiModel = 'mistral-small-latest';
                            }
                            if (_aiProvider == 'OpenRouter') {
                              _aiModel = 'google/gemini-2.0-flash-exp:free';
                            }
                          }
                        });
                      },
                    ...
                    if (_aiProvider == 'Gemini') ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          ...
                          label: const Text(
                            "Gerar API Key Grátis (Google AI Studio)",
                          ),
                    ...
                    if (_aiProvider == 'Claude') ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          icon: const Icon(Icons.open_in_new, size: 16),
                          label: const Text(
                            "Gerar API Key (Anthropic Console)",
                          ),
                          onPressed: () => _launchURL(
                    ...
                    if (_aiProvider == 'DeepSeek') ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          icon: const Icon(Icons.open_in_new, size: 16),
                          label: const Text(
                            "Gerar API Key (DeepSeek Platform)",
                          ),
                          onPressed: () => _launchURL(
                    ...
                    if (_aiProvider == 'Groq') ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          icon: const Icon(Icons.open_in_new, size: 16),
                          label: const Text(
                            "Gerar API Key Grátis (Groq Cloud)",
                          ),
                          onPressed: () =>
                    ...
                    if (_aiProvider == 'Mistral') ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          ...
                          onPressed: () =>
                              _launchURL("https://console.mistral.ai/api-keys"),
                        ),
                      ),
                    ],
                    if (_aiProvider == 'OpenRouter') ...[
                      ...
                      Align(
                        ...
                        child: TextButton.icon(
                          ...
                          onPressed: () =>
                              _launchURL("https://openrouter.ai/settings/keys"),
                        ),
                      ),
                    ],
                      ...
                      children: [
                        Expanded(
                          child: Slider(
                            ...
                            label: "${_replyDelay}s",
                            onChanged: (val) =>
                                setState(() => _replyDelay = val.toInt()),
                          ),
                        ),
                        Text(
                          "${_replyDelay}s",
                    ...
                    const SizedBox(height: 10),
                    const Text(
                      "Esperar por mais mensagens (Agrupamento)",
                      style: TextStyle(fontWeight: FontWeight.bold),
                    ),
                    const Text(
                      "Quanto tempo o bot espera o usuário parar de digitar para responder.",
                      ...
                      children: [
                        Expanded(
                          child: Slider(
                            ...
                            onChanged: (val) =>
                                setState(() => _debounceDelay = val.toInt()),
                          ),
                        ),
                        Text(
                          "${_debounceDelay}s",
                    ...
                    SwitchListTile(
                      ...
                      subtitle: const Text(
                        "Abre o chat e simula a digitação real (mostra 'Digitando...')",
                      ),
                    ...
                    ElevatedButton(
                      ...
                      child: const Text("Salvar Configurações I.A."),
                    ),
                  ],
        ...
        floatingActionButton: Showcase(
          ...
          title: "Criar Regras",
          description:
              "Adicione palavras-chave específicas (ex: 'Preço') e defina o que o bot deve responder automaticamente.",
      ...
      case 'Gemini':
        return const [
          ...
          DropdownMenuItem(
            ...
            child: Text("Gemini 2.0 Flash (Recomendado)"),
          ),
          DropdownMenuItem(
            value: "gemini-2.0-flash-lite-preview",
            child: Text("Gemini 2.0 Flash Lite (Preview)"),
          ),
          DropdownMenuItem(
            value: "gemini-2.0-pro-exp-02-05",
            child: Text("Gemini 2.0 Pro Experimental"),
          ),
          DropdownMenuItem(
          ...
          DropdownMenuItem(
            value: "gemma-3-1b-it",
            child: Text("Gemma 3 1B (API)"),
          ),
          DropdownMenuItem(
            value: "gemma-3-4b-it",
            child: Text("Gemma 3 4B (API)"),
          ),
          DropdownMenuItem(
            value: "gemma-3-12b-it",
            child: Text("Gemma 3 12B (API)"),
          ),
          DropdownMenuItem(
            value: "gemma-3-27b-it",
            child: Text("Gemma 3 27B (API)"),
          ),

          // Specialized / Preview
          DropdownMenuItem(
            value: "gemini-2.5-flash-preview-image",
            child: Text("Nano Banana (2.5 Flash Preview)"),
          ),
          DropdownMenuItem(
            value: "gemini-3-pro-image",
            child: Text("Nano Banana Pro (3.0 Pro Image)"),
          ),
      ...
      case 'Claude':
        return const [
          DropdownMenuItem(
            ...
            child: Text("Claude 3 Haiku (Rápido)"),
          ),
          DropdownMenuItem(
            value: "claude-3-sonnet-20240229",
            child: Text("Claude 3 Sonnet (Equilibrado)"),
          ),
          DropdownMenuItem(
            value: "claude-3-opus-20240229",
            child: Text("Claude 3 Opus (Poderoso)"),
          ),
        ];
      case 'DeepSeek':
        return const [
      ...
      case 'Groq':
        return const [
          DropdownMenuItem(
            ...
            child: Text("Llama 3.1 8B (Meta/Rápido)"),
          ),
          DropdownMenuItem(
            value: "llama-3.3-70b-versatile",
            child: Text("Llama 3.3 70B (Meta)"),
          ),
          DropdownMenuItem(
            value: "mixtral-8x7b-32768",
            child: Text("Mixtral 8x7b (Mistral)"),
          ),
          DropdownMenuItem(
            value: "gemma2-9b-it",
            child: Text("Gemma 2 9B (Google)"),
          ),
        ];
      ...
      case 'Mistral':
        return const [
          DropdownMenuItem(
            ...
            child: Text("Mistral Small (Rápido/Grátis-Tier)"),
          ),
          DropdownMenuItem(
            value: "mistral-large-latest",
            child: Text("Mistral Large (Poderoso)"),
          ),
          DropdownMenuItem(
            value: "open-mistral-nemo",
            child: Text("Mistral Nemo (Forte e Leve)"),
          ),
          DropdownMenuItem(
            value: "open-mixtral-8x22b",
            child: Text("Mixtral 8x22B"),
          ),
          DropdownMenuItem(
            value: "codestral-latest",
            child: Text("Codestral (Programação)"),
          ),
        ];
      ...
      case 'OpenRouter':
        return const [
          DropdownMenuItem(
            ...
            child: Text("Gemini 2.0 Flash Exp (Grátis)"),
          ),
          DropdownMenuItem(
            value: "deepseek/deepseek-r1:free",
            child: Text("DeepSeek R1 (Grátis)"),
          ),
          DropdownMenuItem(
            value: "mistralai/mistral-7b-instruct:free",
            child: Text("Mistral 7B (Grátis)"),
          ),
          DropdownMenuItem(
            value: "meta-llama/llama-3.1-8b-instruct:free",
            child: Text("Llama 3.1 8B (Grátis)"),
          ),
        ];
      default:
        return const [];
