...
class AIService {
  ...
  Future<String?> generateReply(String userMessage, {List<Map<String, String>> history = const []}) async {
    try {
      ...
      if (model == 'google/gemini-2.0-flash-lite-preview-02-05:free' || model == 'google/gemini-2.0-flash-lite-preview') {
      ...
      for (var providerName in fallbackOrder) {
        ...
        if (apiKey == null || apiKey.isEmpty) {
          LoggerService.log("AIService: No API key for $providerName. Skipping.");
          continue;
        }

        // Use the selected model only for the primary provider. 
        // For fallback providers, use a safe default compatible with their SDK.
        final currentModel = (providerName == primaryProvider) ? model : _getDefaultModelFor(providerName);

        AIProvider? provider;
        switch (providerName) {
          case 'Gemini': provider = GeminiProvider(apiKey: apiKey, model: currentModel, prompt: fullPrompt); break;
          case 'Claude': provider = ClaudeProvider(apiKey: apiKey, model: currentModel, prompt: fullPrompt); break;
          case 'DeepSeek': provider = DeepSeekProvider(apiKey: apiKey, model: currentModel, prompt: fullPrompt); break;
          case 'Groq': provider = GroqProvider(apiKey: apiKey, model: currentModel, prompt: fullPrompt); break;
          case 'Mistral': provider = MistralProvider(apiKey: apiKey, model: currentModel, prompt: fullPrompt); break;
          case 'OpenRouter': provider = OpenRouterProvider(apiKey: apiKey, model: currentModel, prompt: fullPrompt); break;
        }

        if (provider == null) continue;

        LoggerService.log("AIService: generating reply using $providerName (Model: $currentModel)...");
        try {
          final response = await provider.generateReply(userMessage, history: history);
          
          if (response != null) {
            if (response.startsWith("Erro")) {
              LoggerService.log("AIService: $providerName failed with error: $response. Trying fallback...");
              continue;
            }
            LoggerService.log("AIService: Successfully generated reply using $providerName.");
            return response;
          } else {
            LoggerService.log("AIService: $providerName returned null response.");
          }
        } catch (e) {
          LoggerService.log("AIService: Exception while generating reply with $providerName: $e");
        }
      }
      ...
      */
      ...
      LoggerService.log("AIService: All providers (online and offline) failed.");
      return null;

    } catch (e, stack) {
      LoggerService.log("AIService: CRITICAL ERROR in fallback loop: $e\n$stack");
      return null;
    }
  }
  ...
  Future<String> _buildPrompt(String basePrompt) async {
    ...
    if (departments.isNotEmpty) {
      ...
      for (var dept in departments) {
        buffer.writeln("- ${dept.name}: ${dept.phoneNumber} (${dept.description})");
      }
      buffer.writeln("Se o usuário solicitar falar com um desses departamentos, forneça o link: https://wa.me/<NUMERO_DO_DEPARTAMENTO> e seja educado.");
    ...
    if (campaignMessage != null && campaignMessage.isNotEmpty) {
      ...
      buffer.writeln("A empresa enviou recentemente uma campanha para seus clientes com o seguinte conteúdo:");
      buffer.writeln("\"$campaignMessage\"");
      if (campaignDate != null) {
        buffer.writeln("Data e hora da campanha: $campaignDate");
      }
      buffer.writeln("IMPORTANTE: Se o cliente responder de forma curta ou perguntar algo que pareça interessado no assunto acima, assuma que ele recebeu essa campanha.");
      buffer.writeln("Sua missão é converter esse interesse em atendimento ou venda baseada no que foi oferecido acima.");
