...
class AutoReplyService {
  ...
  void startListening() {
    ...
    _subscription = _nativeService.notificationStream.listen(
      _handleNotification,
    );
  ...
  Future<void> _handleNotification(Map<String, dynamic> data) async {
    ...
    if (!isAutomationEnabled) {
      LoggerService.log(
        "AutoReply: Automation is disabled via floating toggle. Ignoring notification.",
      );
      return;
    }
    ...
    LoggerService.log(
      "AutoReply: Message from $title buffered (Total: ${_messageBuffers[title]!.length}). Waiting ${debounceSeconds}s...",
    );
  }
  ...
  Future<void> _processReply(
    ...
    if (_processing.contains(title)) {
      LoggerService.log(
        "AutoReply: Skipping process for $title - already processing.",
      );
      return;
    }
    ...
    if (_lastReplyTime.containsKey(title)) {
      ...
      if (DateTime.now().difference(lastTime) < _cooldown) {
        LoggerService.log(
          "AutoReply: Skipping reply to $title - Cooldown active",
        );
        return;
      }
    }
    ...
    final Timer safetyTimer = Timer(const Duration(seconds: 60), () {
      if (_processing.contains(title)) {
        LoggerService.log(
          "AutoReply: Safety fallback triggered - clearing processing for $title",
        );
    ...
    try {
      ...
      if (isAbsenceMode) {
        ...
        LoggerService.log(
          "AutoReply: Absence Mode ON. Sending default message.",
        );
      ...
      LoggerService.log(
        "AutoReply: Checking ${rules.length} rules for message: '$message'",
      );
      ...
      if (replyMessage == null) {
        ...
        if (isAiEnabled) {
          isAiAttempted = true;
          LoggerService.log(
            "AutoReply: No rule matched. Attempting AI reply...",
          );

          // Helper to fetch history
          final history = await _conversationRepository.getHistory(title);

          replyMessage = await _aiService.generateReply(
            message,
            history: history,
          );
          if (replyMessage != null) {
      ...
      if (replyMessage != null) {
        if (replyMessage.startsWith("Erro")) {
          LoggerService.log(
            "AutoReply: AI returned an error: $replyMessage. Sending fallback to client.",
          );
          replyMessage =
              "Olá! Recebi sua mensagem, mas estou processando as informações com um pouco de lentidão. Por favor, aguarde um momento ou tente novamente em instantes.";
        }
        LoggerService.log(
          "AutoReply: Reply generated. Preparing to send to $title...",
        );
        await _sendReply(
          title,
          replyMessage,
        ...
        LoggerService.log(
          "AutoReply: AI was enabled but failed to generate for $title. Sending fallback.",
        );
        final fallback =
            "Olá! No momento não consegui processar sua resposta automaticamente. Um atendente humano verá sua mensagem em breve.";
        ...
        LoggerService.log(
          "AutoReply: No rule matched, AI is disabled, Absence Mode is off. Ignoring message quietly.",
        );
      }
    } catch (e, stack) {
  ...
  Future<void> _sendReply(
    ...
    LoggerService.log(
      "AutoReply: Replying to $title with '$message' (Delay: ${delaySeconds}s)",
    );
    ...
    final success = await _nativeService.replyToNotification(
      title,
      message,
      replyKey: replyKey,
    );
